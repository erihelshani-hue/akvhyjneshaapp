...
import { addDaysToDate, formatDate, formatDateRange, formatTimeRange, getDayDifference } from "@/lib/utils";
import { Edit, MapPin, Clock, ArrowLeft, CalendarDays } from "lucide-react";
...
export default async function RehearsalDetailPage({
  ...
  const endDateOffset = rehearsal.end_date ? getDayDifference(rehearsal.date, rehearsal.end_date) : 0;
  const targetEndDate = endDateOffset > 0 ? addDaysToDate(targetDate, endDateOffset) : targetDate;
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-foreground">
            {formatDateRange(targetDate, targetEndDate)}
          </span>
        </div>
        <div className="flex items-center gap-2 text-sm">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-foreground">{rehearsal.location}</span>
        </div>
        {notes && (
