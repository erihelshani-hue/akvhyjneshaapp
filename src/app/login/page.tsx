...
function messageForErrorCode(errorCode: string | null, errorDescription: string | null): string | null {
  if (!errorCode && !errorDescription) return null;
  switch (errorCode) {
    case "otp_expired":
      return "Der Anmeldelink ist abgelaufen. Bitte fordere einen neuen an.";
    case "access_denied":
      return "Der Link wurde bereits verwendet oder ist ungültig. Bitte fordere einen neuen an.";
    case "over_email_send_rate_limit":
    case "rate_limit":
      return "Zu viele Anfragen. Bitte warte eine Minute und versuche es erneut.";
    case "missing_code":
      return "Der Anmeldelink ist unvollständig. Bitte fordere einen neuen an.";
    case "exchange_failed":
      return errorDescription ?? "Anmeldung fehlgeschlagen. Bitte fordere einen neuen Link an.";
    default:
      return errorDescription ?? "Anmeldung fehlgeschlagen. Bitte versuche es erneut.";
  }
}
...
function LoginPageInner() {
  ...
  // Surface callback/Supabase errors passed back via the URL.
  useEffect(() => {
    const msg = messageForErrorCode(
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Use window.location.origin so preview deployments and custom domains both work.
    // Falls back to NEXT_PUBLIC_APP_URL only during SSR (should never run here — "use client").
    const origin =
    ...
    if (otpError) {
      setError(messageForErrorCode(otpError.code ?? null, otpError.message) ?? t("error"));
    } else {
      setSent(true);
    }
    setLoading(false);
